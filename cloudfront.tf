...
#############################################
locals {
  # Clean unified CloudFront origin-id namespace
  origin_id = "${var.service_name}-${var.env_name}-cdn"
}

#############################################
# CACHE POLICIES (AWS MANAGED)
#############################################
...
resource "aws_cloudfront_response_headers_policy" "security" {
  ...
  name     = "${var.service_name}-${var.env_name}-security-${each.key}"
  ...
  # Only STATIC needs CORS override
  dynamic "cors_config" {
    for_each = each.key == "static" ? [1] : []
    content {
      ...
      access_control_allow_origins {
        items = [
          "https://${var.domain_name_internal}",
          "https://${var.domain_name_external}"
        ]
      }
...
resource "aws_cloudfront_distribution" "media" {
  ...
  enabled    = true
  web_acl_id = aws_wafv2_web_acl.panelapp.arn
...
resource "aws_cloudfront_distribution" "static" {
  ...
  tags = {
    Name = "${var.service_name}-${var.env_name}-static-cdn"
  }
}
...
resource "aws_s3_bucket" "cloudfront_logs" {
  bucket        = "${var.bucket_name}-cloudfront-logs-${var.env_name}"
  force_destroy = true
}
